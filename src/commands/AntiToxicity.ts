...
export default class AntiToxicity extends Command {
  ...
  options = [
    {
      ...
      name: "set",
      description: "Configure anti-toxicity auto-mod filter.",
      options: [
        {
    ...
    },
    {
      type: OptionType.SUB_COMMAND,
      name: "disable",
      description: "Disable the anti-toxicity automod filter.",
    },
  ];
  guildID = null;
  dmPermission = false;
  ...
  async handler(interaction: CommandInteraction<GuildTextableChannel>) {
    ...
    if (subCommand.name === "set" && subCommand.options) {
          ...
          .then(() => {
            return interaction.editOriginalMessage({
              content: this.successMessage(
                ...
                "Updated the anti-toxicity automod filter."
              ),
              components: [],
            });
          })
      ...
      return interaction.createFollowup(
        this.successMessage(interaction.channel, "Disabled the anti-toxicity automod filter.")
      );
    }
  }
}
